...
SOURCE = ROOT / "web/openclaw-sprites"
OUTPUT = ROOT / "assets/openclaw-lab.bin"
METADATA = ROOT / "assets/openclaw-lab.json"
HEADER = ROOT / "firmware/Copilot/generated/openclaw_assets.h"
TRACKS = (
...
def main():
    ...
    lines = [
        "// Generated by tools/export_openclaw_lab.py; do not edit.",
        "#pragma once",
        "#include <cstdint>",
        "namespace copilot {",
        "struct OpenClawSpriteBlock { uint32_t offset, size; };",
        f"constexpr uint8_t kOpenClawDirections = {len(TRACKS)};",
        f"constexpr uint8_t kOpenClawSteps = {STEPS};",
        f"constexpr uint8_t kOpenClawBlinkLevels = {BLINKS};",
        f"constexpr uint32_t kOpenClawOpenDataSize = {group_ends[0]};",
        f"constexpr uint32_t kOpenClawDataSize = {len(store.data)};",
        "constexpr uint8_t kOpenClawDataSha256[32] = {",
        "  " + ", ".join(f"0x{byte:02x}" for byte in hashlib.sha256(store.data).digest()) + ",",
        "};",
        f"constexpr OpenClawSpriteBlock kOpenClawFrames[{len(blocks)}] = {{",
    ]
    ...
    metadata = {
        ...
        "modelSha256": digest(ROOT / "tools/openclaw/model.js"),
        "exporterSha256": digest(ROOT / "tools/openclaw/export-sprites.mjs"),
        "packExporterSha256": digest(ROOT / "tools/export_openclaw_lab.py"),
        "generatedHeaderSha256": digest(HEADER),
        "packageLockSha256": digest(ROOT / "tools/openclaw/package-lock.json"),
        "referenceSvgSha256": digest(ROOT / "assets/openclaw/reference.svg"),
